...
![星夜营地](docs/images/screenshot2.png)
...
- **打野(自由专注)**:不想被时长束缚时,点「打野 · 自由专注」开一局开放式专注,计时正向累加,随时结束按实际分钟结算(与委托同率、单次封顶 120 分钟、不足 1 分钟不结算、无撤退惩罚)。
- **讨伐(Boss × 代办)**:把大目标(如"啃完一本书")立成讨伐旗,拆成带时长的代办;选一条专注即勾掉并扣血,清空全部代办即击败,获 金币 + 高稀有蛋(随目标总时长升级) + 专属称号。代办也可改用「打野」按实际时长清。
...
- **营地小玩法**(金币出口):「星夜转盘」抽卡——概率得材料/蛋,十连含保底;「营地农园」种作物——每完成一次专注 +1 成长,成熟收材料(晶花概率出蛋);「篝火骰戏」押大小赌金币——每日 10 局上限,只赌金币、绝不碰已得资产。
- **成就**:在「统计」页查看 12 项成就(首次出击、累计时长/次数、升级、点亮图鉴、首杀讨伐等),达成即可一键领取金币 / 蛋奖励——只按你已有的数据判定,不增加额外任务负担。
- **日夜与设置**:界面随本地时间切换日/夜主题(6:00–18:00 日间);⚙️ 设置可锁定主题、开关音效、调节动效(跟随系统/开/关——系统开了「减少动态效果」时可在此强制恢复场景动效)。
...
> 打野(自由专注)与委托**同率结算**,按实际专注分钟计,单次封顶 120 分钟、不足 1 分钟不结算。
...
```bash
npm test           # server(77)+ client(4)全部测试
```

## 路线图

- **v2**:AI 生成每日小剧情与随机事件、营地装饰自由摆放、手机端优化、数据导出。

> 已上线:打野 · 讨伐(Boss×代办) · 星夜转盘 · 营地农园 · 篝火骰戏 · 成就系统 · 动效开关(详见「玩法」)。
